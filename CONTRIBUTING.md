# Contributing to ep_file_menu_toolbar

Thanks for helping improve `ep_file_menu_toolbar` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_file_menu_toolbar` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_file_menu_toolbar from your local clone
pnpm run plugins i --path ../ep_file_menu_toolbar

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_file_menu_toolbar/` are picked up after a server restart.
...
* Etherpad version, ep_file_menu_toolbar version, browser + OS
* Steps to reproduce

  ...
  $.fn.extend({
    dropdown_menu(options) {
      const _defaults = {
        ...
        init() {}, // Callback function applied on init
      };

      // Test for IE <= 7
      version = parseFloat(navigator.appVersion.split('MSIE')[1]);
      const ie7 = (version < 8) && (document.body.filters);

      return this.each(function () {
        const menu = $(this);
        // Needs this single/double quote precedence for JSON data
        // <ul data-options='{"sub_indicators":"true"}'></ul>
        const metadata = menu.data('options');
        const o = $.extend({}, _defaults, options, metadata);

        // Arrow element
        const sub_indicator = $(`<span class="${o.sub_indicator_class}">&#187;</span>`);
        ...
        menu.find('li:has(ul)').each(function () {
          ...
          const submenu = $('>ul', this).css({visibility: 'hidden', display: 'block'});
          ...
          $(this).on({
            mouseenter(e) {
              ...
              const rootElement = $(e.fromElement).parent().parent().attr('class');
              if (rootElement) {
                ...
                if (overflow && o.viewport_overflow) {
                  ...
                  switch (o.viewport_overflow) {
                    case 'move':
                      ...
                      submenu.offset({left, top});
                      break;
                    case 'scroll':
                      if (overflow_x) {
                        const scrollLeft = submenu.offset().left - $(window).width() + submenu.width() + padding;
                        $('html').animate({scrollLeft}, 'fast');
                        // $(window).scrollLeft(scrollLeft);
                      }
                      if (overflow_y) {
                        const scrollTop = submenu.offset().top - $(window).height() + submenu.height() + padding;
                        $('html').animate({scrollTop}, 'fast');
